  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `https://insight.thirdweb.com/v1/events/0x2a61627c3457cCEA35482cAdEC698C7360fFB9F2?chain=137&sort_by=block_timestamp&sort_order=desc&limit=100&clientId=${process.env.NEXT_PUBLIC_CLIENT_ID}`
      );
              ...
              {data
                ...
                .map((decoded, index) => (
                  <tr key={index}>
                    ...
                    <td className="border border-gray-500 p-2">{new Date(decoded.block_timestamp * 1000).toLocaleString()}</td>
                  </tr>
                ))}
            </tbody>
